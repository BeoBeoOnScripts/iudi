...
def createUsers():
    try:
        ...
        if not isValidEmail(json_data["Email"]):
            return make_response(
                jsonify({"Status": 400, "message": "Invalid email format."})
            )
        ...
        if user:
            ...
            return make_response(
                jsonify({"Status": 400, "message": "Username or Gmail exist."})
            )
        ...
        print(f"Error: {e}")
        return make_response(
            jsonify({"Status": 500, "message": "Username and Gmail exist."})
        )
    ...
    if request.method == "PUT":
        try:
            ...
            if user:
                ...
                db.session.commit()
                return make_response(
                    jsonify({"status": 200, "message": "User updated successfully"})
                )
            else:
                return make_response(
                    jsonify({"status": 404, "message": "User not found"}), 404
